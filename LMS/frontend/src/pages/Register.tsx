...
export default function Register() {
  ...
  return (
    <div className="lms-auth flex min-h-screen items-center justify-center px-6 text-white">
      <button onClick={() => navigate("/")} className="absolute left-6 top-6 flex items-center gap-2 text-sm text-white/50 hover:text-white">
        <ArrowLeft size={17} />Back
      </button>
      <div className="w-full max-w-md">
        <div className="mb-8 text-center">
          <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-white text-black"><BookOpen size={27} /></div>
          <h1 className="text-3xl font-semibold">Create your account</h1>
          <p className="mt-2 text-sm text-white/40">Join the LMS platform</p>
          <div className="mt-4 inline-flex items-center rounded-full border border-violet-400/15 bg-violet-400/5 px-3 py-1.5 text-xs text-violet-200">Section allocation is managed by Admin</div>
        </div>
        ...
        <form onSubmit={register} className="rounded-3xl border border-white/10 bg-white/[0.035] p-8 backdrop-blur-xl">
          ...
          {message && <div className="mt-5 rounded-xl border border-green-400/20 bg-green-400/10 px-4 py-3 text-sm text-green-300">{message}</div>}
          <button type="submit" disabled={loading} className="lms-btn-primary mt-7 w-full rounded-xl py-3 text-sm font-medium disabled:opacity-50">{loading ? "Creating account..." : "Create Account"}</button>
          <p className="mt-6 text-center text-sm text-white/40">Already have an account?{" "}<button type="button" onClick={() => navigate("/login")} className="text-white hover:underline">Sign in</button></p>
        </form>
      </div>
    </div>
  )
