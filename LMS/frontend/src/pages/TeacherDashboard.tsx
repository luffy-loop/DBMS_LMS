...
function Field({label,children}:{label:string;children:ReactNode}){return <div><label className="mb-2 block text-sm font-medium text-white/60">{label}</label>{children}</div>}
function Stat({icon,title,value,detail}:{icon:ReactNode;title:string;value:number;detail:string}){return <div className="lms-stat rounded-2xl p-5"><div className="flex items-start justify-between"><div className="lms-icon flex h-10 w-10 items-center justify-center rounded-xl">{icon}</div><span className="text-xs text-white/20">LIVE</span></div><p className="mt-5 text-sm text-white/40">{title}</p><p className="mt-1 text-2xl font-semibold">{value}</p><p className="mt-1 text-xs text-white/25">{detail}</p></div>}
